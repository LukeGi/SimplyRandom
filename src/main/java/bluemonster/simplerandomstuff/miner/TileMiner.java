...
import bluemonster.simplerandomstuff.core.IHaveGui;
...
public class TileMiner extends TileEntity implements ITickable, IHaveGui {
    ...
    private void updateClient() {
        ...
        if (currentPosition != getPos()) {
            render = BoxRender.create(
                    new Color(200, 30, 30, 30),
                    new Vec3d(currentPosition),
                    new Vec3d(currentPosition.add(1, 1, 1)),
                    BoxRender.BoxMode.NORMAL
            );
            render.show();
        }
    }
    ...
    private void destroyBlock() {
        if (battery.getEnergyStored() >= FRMiner.INSTANCE.getBreakPower() && canDestroy()) {
            battery.extractEnergy(FRMiner.INSTANCE.getBreakPower(), true);
            IBlockState blockState = getWorld().getBlockState(currentPosition);
            NonNullList<ItemStack> dropsList = NonNullList.create();
            blockState.getBlock().getDrops(dropsList, getWorld(), currentPosition, blockState, 0);
            dropsList.addAll(this.drops);
            if (getWorld().destroyBlock(currentPosition, false)) {
                this.drops.addAll(dropsList);
            }
        }
    }

    private boolean canDestroy() {
        if (battery.getEnergyStored() >= FRMiner.INSTANCE.getScanPower()) {
            battery.extractEnergy(FRMiner.INSTANCE.getScanPower(), true);
    ...
    private void moveToStartingPosition() {
        ...
        currentPosition = new BlockPos(
                currentPositionX - currentPositionX % 15,
                currentPosition.getY() - 1,
                currentPositionZ - currentPositionZ % 15
        );
    }

    private void checkIfCanWork() {
        if (getNextPosition(currentPosition).getY() <= 0 || battery.getEnergyStored() <= FRMiner.INSTANCE.getScanPower()) {
            canWork = false;
            return;
        }
    ...
    public void readFromNBT(NBTTagCompound tag) {
        if (tag.hasKey(NBT_CURRENT_POS)) {
            currentPosition = BlockPos.fromLong(tag.getLong(NBT_CURRENT_POS));
        } else {
            currentPosition = getPos();
        }
        ticker.readFromNBT(tag);
        battery.setEnergy(tag.getInteger(NBT_ENERGY_STORED));
        inventory.deserializeNBT(tag.getCompoundTag(NBT_INVENTORY));
        if (tag.hasKey(NBT_DROPS)) {
            ItemStackHandler handler = new ItemStackHandler(20);
            handler.deserializeNBT(tag.getCompoundTag(NBT_DROPS));
    ...
    public NBTTagCompound writeToNBT(NBTTagCompound tag) {
        if (currentPosition != null && currentPosition != getPos()) {
            tag.setLong(NBT_CURRENT_POS, currentPosition.toLong());
        }
        ticker.writeToNBT(tag);
        tag.setInteger(NBT_ENERGY_STORED, battery.getEnergyStored());
        ...
        if (!drops.isEmpty()) {
            ...
            tag.setTag(NBT_DROPS, handler.serializeNBT());
        }
