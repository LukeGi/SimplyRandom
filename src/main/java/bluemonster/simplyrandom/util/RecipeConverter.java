...
import bluemonster.simplyrandom.SimplyRandom;
...
// Also works with OD, replace GameRegistry.addRecipe(new ShapedOreRecipe/ShapelessOreRecipe with the same calls
    ...
    private static void setupDir() {
        if (RECIPE_DIR == null) {
            RECIPE_DIR = new File("C:\\Users\\boo12\\Documents\\Programming\\SimplyRandom\\src\\main\\resources\\assets\\simplyrandom\\recipes");
        }
    ...
    public static void addShapedRecipe(ItemStack result, Object... components) {
        setupDir();
        if (!doWork) return;
        SimplyRandom.logger.info("Adding Recipe for " + result.toString());
