  ...
  implements ITickable, IHaveGui
{
  ...
  private void moveScanner()
  {
    ...
    //SRS.INSTANCE.logger.info(currentPosition.toString());
  }
  ...
  private void destroyBlock()
  {
    if (battery.getEnergyStored() >= FRMiner.BREAK_POWER && canDestroy())
    {
      ...
      List<ItemStack> dropsList  = blockState.getBlock()
                                             .getDrops(getWorld(), currentPosition, blockState, 0);
  ...
  {
    if (tag.hasKey("currentPosition"))
    {
      currentPosition = BlockPos.fromLong(tag.getLong("currentPosition"));
    }
    else
    {
      currentPosition = getPos();
