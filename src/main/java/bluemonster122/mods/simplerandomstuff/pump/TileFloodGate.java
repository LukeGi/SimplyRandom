...
public class TileFloodGate
  extends TileST
  implements IHaveTank, ITickable
{
  ...
  private boolean placeFluidAt(BlockPos fillPos)
  {
    if (isValidBlock(fillPos) || isFlowing(fillPos))
    {
      ...
      {
        BlockDynamicLiquid block = BlockLiquid.getFlowingBlock(b.getDefaultState()
                                                                .getMaterial());
   ...
   *
   * @param remove_value
   *   this should be true, if you want the method to poll the list, it will remove the value from being usable again.
   *
  ...
  private BlockPos getNextSpot(boolean remove_value)
  {
    ...
    Deque<BlockPos> bottom_layer = layersToFill.firstEntry()
                                               .getValue();
  ...
  private void queueForFilling(BlockPos pos)
  {
    ...
    if (visited.add(pos))
    {
      ...
      if (handler != null)
      {
        ...
        if (stack != null)
        {
          if (stack.getFluid() == tank.getFluid()
                                      .getFluid())
          {
            fluidBlocks.add(pos);
          }
  ...
  {
    layersToFill.computeIfAbsent(pos.getY(), k -> new LinkedList<>())
                .addLast(pos);
  }
  ...
  private boolean isValidBlock(BlockPos pos)
  {
    ...
    return (state.getMaterial()
                 .isReplaceable() && FluidUtil.getFluidHandler(
      getWorld(),
      pos,
      null
    ) == null) || state.getMaterial() == Material.AIR;
  }
