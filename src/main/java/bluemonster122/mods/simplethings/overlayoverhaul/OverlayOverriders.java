...
public class OverlayOverriders {
    ...
    public static boolean HealthGui(Minecraft mc) {
        ...
        boolean inCombat = !player.world.<EntityMob>getEntitiesWithinAABB(EntityMob.class, new AxisAlignedBB(player.getPosition()).expand(8, 5, 8)).isEmpty();
        ...
        if (player.getAbsorptionAmount() > 0) {
            Gui.drawModalRectWithCustomSizedTexture(left, top, 0, 35, (int) (barWidth * (player.getAbsorptionAmount() / 20f)), barHeight, 256, 256);
        }
    ...
    public static void FoodGui(Minecraft mc) {
        ...
        GlStateManager.disableBlend();
        Gui.drawModalRectWithCustomSizedTexture(left, top, 0, y, Math.round(smallBarWidth * perc), smallBarHeight, 256, 256);
        if (saturationLevel > 0f) {
            perc = saturationLevel / 20f;
            y = 45;
            Gui.drawModalRectWithCustomSizedTexture(left - 1, top - 1, 0, y, Math.round((smallBarWidth + 2) * perc), smallBarHeight + 2, 256, 256);
        }
        mc.getTextureManager().bindTexture(Gui.ICONS);
    }
    ...
    public static void XPGui(Minecraft mc) {
        ...
        Gui.drawModalRectWithCustomSizedTexture(left, top, 0, 253, smallBarWidth, smallBarHeight, 256, 256);
        GlStateManager.disableBlend();
        Gui.drawModalRectWithCustomSizedTexture(left, top, 0, 134, Math.round(smallBarWidth * experience), smallBarHeight, 256, 256);
        mc.getTextureManager().bindTexture(Gui.ICONS);
    }
    ...
    public static void ArmorGui(Minecraft mc) {
        ...
        GlStateManager.disableBlend();
        // Draw the armor bar
        Gui.drawModalRectWithCustomSizedTexture(left, top, 0, 140, Math.round(smallBarWidth * armor), smallBarHeight, 256, 256);
        // Draw the armor damage bar
        Gui.drawModalRectWithCustomSizedTexture(left, top, 0, 137, Math.round(smallBarWidth * armorDurability), smallBarHeight, 256, 256);
    ...
    public void levelup(PlayerPickupXpEvent event) {
        ...
        if (FMLCommonHandler.instance().getSide().isClient()) {
            ...
            event.getEntityPlayer().addExperience(event.getOrb().xpValue);
            event.getOrb().setDead();
            if (event.getEntityPlayer().experienceLevel != level) {
                event.getEntityPlayer().sendStatusMessage(new TextComponentString("You are now Level " + level), true);
            }
            event.setCanceled(true);
        }
    }
