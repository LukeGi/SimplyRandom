...
public class TreeChoppa {
    ...
    private void cutTree(Set<BlockPos> tree) {
        tree = tree.stream().sorted().collect(Collectors.toSet());
        ...
        chunksUpdated.forEach(Chunk::setChunkModified);
        ...
        for (int i = 0; i < dropInv.getSlots(); i++) {
            ...
            if (leftovers != ItemStack.EMPTY) {
                EntityItem entityIn = new EntityItem(world, farmNew.getPos().getX() + 0.5, farmNew.getPos().getY() + 5, farmNew.getPos().getZ() + 0.5, leftovers);
